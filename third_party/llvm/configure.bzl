...
def _extract_cmake_settings(repository_ctx, llvm_cmake):
    ...
    #
    # Local modification: upstream uses `Label("//:" + llvm_cmake)`, a
    # self-reference to the repository currently being generated.  Under Bazel 8
    # + Bzlmod that triggers a package load of the not-yet-fetched repo and
    # fails ("BUILD file not found").  The overlay step already materialized the
    # source tree at the repo root, so resolve the file by a plain root-relative
    # path instead, which needs no package loading.
