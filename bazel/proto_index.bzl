...
`bazel/integration/proto_index.bzl` is the kit's copy of this file, for the
prebuilt binary; change the two together.

The index is one format with a producer per language (see
`cpp_formatting/index.proto`).  `cpp_index_aspect` in `cpp_format.bzl` is the
C++ one; this is the second, and it lives in a file of its own so that a
repository without a single `.proto` never loads `@protobuf` on the index's
account: `cpp_format.sh` names this aspect only when its query finds a
`proto_library`.
