...
# cross-target reference: demo_main.cpp (a cc_binary) uses Widget::item_count,
...
grep -q 'usr: "c:@S@Widget@FI@item_count"' "$out/index.txt" \
  || fail "Widget::item_count not in the index"
...
off_use="$(offset_of "$demo_main" 'item_count = 4')"
off_def="$(offset_of "$demo_h" 'item_count;')"
"$cpp_format" --dump-index --lookup="bazel/testdata/demo_main.cpp:$off_use" "$index" \
  > "$out/lookup.txt"
cat "$out/lookup.txt"
grep -q '^c:@S@Widget@FI@item_count$' "$out/lookup.txt" \
  || fail "lookup in demo_main.cpp did not resolve to Widget::item_count"
grep -q "^  bazel/testdata/demo.h:$off_def-$((off_def + 10)) DEFINITION$" "$out/lookup.txt" \
  || fail "definition in demo.h not listed"
grep -q "^  bazel/testdata/demo_main.cpp:$off_use-$((off_use + 10)) REFERENCE|WRITE$" "$out/lookup.txt" \
  || fail "use in demo_main.cpp not listed"
echo "PASS: cross-target lookup"

# `w.item_count` in demo.h's template is a dependent token: demo.h's own
...
off_dep="$(offset_of "$demo_h" 'return w.item_count')"
off_dep=$((off_dep + 9))
grep -q '^unresolved {' "$out/index.txt" && fail "an unresolved dependent token remains"
grep -q "^  bazel/testdata/demo.h:$off_dep-$((off_dep + 10)) REFERENCE|DEPENDENT$" "$out/lookup.txt" \
