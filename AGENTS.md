...
**How it works** — a per-target **aspect** (`cpp_format_aspect`) derives each `cc_*` target's compile flags from `CcInfo.compilation_context` + the toolchain (`cc_common.get_memory_inefficient_command_line(CPP_COMPILE_ACTION_NAME)`), and runs `cpp_format --config=cpp_format.yaml --emit-edits=<target>.cpp_format.json --jobs=<bucket> <srcs+hdrs> --owned-files=<param file> -- -x c++ <flags> -resource-dir=<staged builtin headers>`. It declares the target's sources **plus `compilation_context.headers` (transitive)** and `cc_toolchain.all_files` as action inputs — that declaration is what makes headers reachable under sandboxing. Each action is parallel and cached; first-party targets only (guarded by `ctx.label.workspace_name == ""`).

**Threads inside an action (`--jobs` + `resource_set`)** — Bazel parallelizes across targets, but one action parses every src *and* hdr of its target, so a big target (googletest's `gtest`, ~60 TUs) used to be a serial tail. The aspect now passes `--jobs=<bucket>`, the largest of 1/2/4/8/16 not above the number of sources (`_jobs_for`), and sets `resource_set` to the matching top-level `_rs_<n>` function (`{"cpu": n, "memory": 300*n}`), so Bazel's scheduler accounts for the threads instead of admitting CPU-count actions that each spawn CPU-count threads. Buckets because a `resource_set` callback must be a top-level function (no lambdas). Bazel 8 honours `resource_set` unconditionally (`--experimental_action_resource_set` is a no-op); an action asking for more CPUs than the host has runs when nothing else does, and the binary caps `--jobs` at the CPU count anyway. The memory figure is a hint. Since the aspect passes a flag an older published binary rejects, this is one of the "aspect and binary are versioned together" changes (see below).
