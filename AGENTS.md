...
- [bazel/testdata/index_test.sh](bazel/testdata/index_test.sh) — reads `//bazel/testdata:index.index` (built by the `cpp_index_targets` rule over `demo` and `demo_main`) back with `--dump-index` and asserts exec-root-relative paths and the cross-target lookup: the use in `demo_main.cpp` resolves to `Widget::item_count` with its definition in `demo.h`, and the dependent `w.item_count` in `demo.h`'s template is listed as `REFERENCE|DEPENDENT` — resolved by the actions of the two files that instantiate it, with nothing left unresolved.
...
**`textual_hdrs` are invisible** — the aspect passes each target's `srcs` and
`hdrs`; a file listed in `textual_hdrs` is in neither, so it is never parsed as
a TU and never named as an owned file. Its references are therefore missed, and
a member it names is renamed at the declaration and left behind there — the
same failure mode as a macro body, but without the veto, because nothing ever
sees the reference. abseil's `absl/log/log_basic_test_impl.inc` naming
`LogEntry::kNoVerbosityLevel` is the measured case
(`abseil_cpp-member_snake_case`). Adding them to the action's inputs is not
enough on its own: a textual header is not self-contained (that is why it is
not a `hdr`), so it cannot be parsed as its own TU, and it would have to be
reached as an owned file of the targets that include it.
...
- **A dependent-token veto does not reach its declaration** — the all-or-nothing coupling that makes a rename safe works through `RenameVeto`, which is keyed on the *owning declaration*, so `mergeEditReports` can drop every edit that belongs to it. A dependent token that cannot be resolved does **not** go through that channel: `vetoResolution()` marks the token's `(file, offset)` and records no owner, so aggregation drops that one token's edit and the declaration's rename survives — the declaration is renamed and the dependent use is left spelling the old name. `abseil_cpp-member_snake_case` is the corpus case; reduced:

  ```cpp
  template <class T> struct Data { union { Status status_; }; };
  template <class T> struct StatusOr : Data<T> {
    bool ok() const { return this->status_.ok(); }   // not rewritten
  };
  ```
