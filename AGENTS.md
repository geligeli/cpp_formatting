...
- The `patches/` directory holds patches applied to the LLVM source at fetch time, listed in the `llvm-raw` `http_archive` in [third_party/llvm/extensions.bzl](third_party/llvm/extensions.bzl). All four are Windows/MSVC fixes: three keep zlib/zstd/blake3 building under MSVC, and `llvm_trailing_objects_friend_msvc.patch` rewrites `friend class TrailingObjects;` to `friend TrailingObjects;` in `TrieRawHashMap.cpp`, which MSVC otherwise rejects with C2990 (it resolves the injected-class-name to the namespace-level template). Only `llvm_smallvector_cstdint.patch` is unused — that one went obsolete with the LLVM 19 upgrade, since upstream `SmallVector.h` now includes `<cstdint>` itself. Clang/LLVM is built from source via the module extension in [third_party/llvm/](third_party/llvm/); see the "Clang/LLVM from source" section above.
