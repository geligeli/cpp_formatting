...
- **Coverage:** `bazel coverage //...` (no config needed) writes LCOV to `bazel-out/_coverage/_coverage_report.dat`, for `//cpp_formatting` and `//code_browser`. Binaries a test runs as subprocesses count, even if they are killed. Works on x86_64 only (see [docs/build.md](docs/build.md)).
