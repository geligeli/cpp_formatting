...
- [bazel/testdata/index_test.sh](bazel/testdata/index_test.sh) — reads `//bazel/testdata:index.index` (built by the `cpp_index_targets` rule over `demo` and `demo_main`) back with `--dump-index` and asserts exec-root-relative paths and the cross-target lookup: the use in `demo_main.cpp` resolves to `Widget::item_count_` with its definition in `demo.h`, and the dependent `w.item_count_` in `demo.h`'s template is listed as `REFERENCE|DEPENDENT` — resolved by the actions of the two files that instantiate it, with nothing left unresolved.
...
Supported scopes: `member`, `local`, `global`, `static_member`, `const_member`, `static_global`, `const_global`, `method`, `type` (classes, structs, unions, enums, class templates, typedefs and aliases), `namespace` (named namespaces and namespace aliases).
